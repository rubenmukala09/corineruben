...
export function useDashboardMetrics() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const daysProtected = Math.floor((Date.now() - createdAt.getTime()) / (1000 * 60 * 60 * 24));
      ...
      const protectionScore = Math.min(baseScore + threatBonus + trainingBonus, 100);

      // Calculate overall training progress
      const totalProgress = enrollments?.reduce((acc, e) => acc + (e.progress_percentage || 0), 0) || 0;
      const overallProgress = enrollments?.length ? Math.round(totalProgress / enrollments.length) : 0;
