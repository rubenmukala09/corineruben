    ...
    <div className="min-h-screen pt-28 pb-20 relative overflow-hidden gradient-mesh">
      ...
      <div className="container mx-auto px-4 max-w-3xl relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
          ...
          <h1 className="font-serif-display text-4xl md:text-6xl text-foreground mb-4 font-bold">{t('story.title')}</h1>
          <p className="font-sans-elegant text-base text-muted-foreground max-w-md mx-auto">{t('story.subtitle')}</p>
        </motion.div>
            ...
            >
              ...
              <div className={`ml-14 md:ml-0 md:w-[calc(50%-2rem)] ${i % 2 === 0 ? 'md:pr-8 md:text-right' : 'md:pl-8'}`}>
                <div className="glass-card-strong rounded-2xl p-6 hover:shadow-card-hover transition-all duration-500">
                  ...
                  <h3 className="font-serif-display text-xl text-foreground mt-2 mb-3 font-bold">{t(event.titleKey)}</h3>
                  <p className="font-sans-elegant text-sm text-muted-foreground leading-relaxed">{t(event.descKey)}</p>
                </div>
              </div>
            </motion.div>
          ))}
