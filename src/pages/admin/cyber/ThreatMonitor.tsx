...
export default function ThreatMonitor() {
  ...
  const addThreatMutation = useMutation({
    mutationFn: async (threat: {
      ...
      const { data, error } = await supabase
        .from("threat_events")
        .insert({ ...threat, profile_id: user.id, status: "active" })
        .select()
        .single();
      if (error) throw error;
      return data;
  ...
  const blockedToday = threats.filter(
    (t) =>
      new Date(t.created_at).toDateString() === new Date().toDateString() &&
      t.status === "resolved",
  ).length;
  const underReview = threats.filter(
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setNewThreat((prev) => ({ ...prev, severity: value }))
                  }
                >
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-[#9CA3AF]">No threat events detected.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {threats.map((threat) => (
                <div
                  key={threat.id}
                  className="flex items-center justify-between p-4 rounded-lg bg-[#1F2937] border border-gray-800 hover:border-gray-700 transition-colors"
                >
                  ...
                  <div className="flex items-center gap-4">
                    <span
                      className={`px-2 py-1 rounded text-xs font-medium capitalize ${getSeverityColor(threat.severity)}`}
                    >
                      {threat.severity}
                    </span>
