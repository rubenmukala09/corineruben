...
export default function DatabaseView() {
  ...
  const loadDatabaseStats = async () => {
    try {
      ...
      const [profiles, threats, activity, subscriptions, newsletters] =
        await Promise.all([
        ...
        <CardContent>
          <div className="space-y-4">
            {databases.map((db) => (
              <div
                key={db.id}
                className="flex items-center justify-between p-4 rounded-lg bg-[#1F2937] border border-gray-800"
              >
