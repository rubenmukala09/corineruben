  ...
  const filteredClients = mockClients.filter((client) => {
    ...
    const matchesService =
      ...
      client.services.some(
        (s) => s.toLowerCase().replace(" ", "-") === serviceFilter,
      );
      ...
      return (
        <div className="flex gap-1 flex-wrap">
          {services.map((service) => (
            <span
              key={service}
              className={`px-2 py-1 rounded-full text-xs font-medium border ${colors[service]}`}
            >
              {service}
            </span>
          ))}
              ...
              <TableBody>
                {filteredClients.map((client) => (
                  <TableRow
                    ...
                    onClick={() =>
                      navigate(`/admin/clients/businesses/${client.id}`)
                    }
                  >
                    ...
                    <TableCell className="font-semibold text-[#F9FAFB]">
                      {client.companyName}
                    </TableCell>
                    <TableCell>
                      <div>
                    ...
                    <TableCell className="font-bold text-lg text-[#F9FAFB]">
                      ${client.mrr}
                    </TableCell>
