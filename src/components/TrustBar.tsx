      ...
      <div className="container mx-auto">
        <div className="relative mx-auto max-w-5xl animate-fade-in-up group/trust">
          ...
          <div className="absolute inset-0 rounded-3xl overflow-hidden">
            <div 
              className="absolute inset-[-2px] rounded-3xl opacity-75 animate-spin"
              style={{
          ...
          {/* Main content */}
          <div className="relative bg-gradient-to-r from-primary to-accent rounded-3xl py-5 px-6 sm:px-10 shadow-[0_12px_40px_rgba(139,92,246,0.5)] backdrop-blur-sm border border-accent/30 group-hover/trust:shadow-[0_20px_60px_rgba(139,92,246,0.7)] group-hover/trust:scale-[1.02] transition-all duration-500">
            ...
            {trustIndicators.map((item, index) => (
              <div key={index} className="flex items-center gap-1.5 sm:gap-2 group animate-fade-in-up flex-shrink-0" style={{ animationDelay: `${index * 100}ms` }} role="listitem">
                <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-lg bg-white/30 backdrop-blur-sm flex items-center justify-center group-hover:scale-125 group-hover:rotate-12 transition-all duration-500 group-hover:bg-white/40 group-hover:shadow-glow-teal" aria-hidden="true">
