...
const Navigation = () => {
  ...
  const mainNavLinks = [
    { name: "Home", href: "/" },
    { name: "For Families", href: "/training" },
  ];

  const businessSubLinks = [
    { name: "AI Receptionist", href: "/business/ai-receptionist" },
    { name: "AI Automation", href: "/business/ai-automation" },
    { name: "Website Design", href: "/business/website-design" },
    { name: "Website Insurance", href: "/business/website-insurance" },
  ];
  ...
  const allMobileLinks = [
    ...
    ...businessSubLinks.map(link => ({ ...link, name: `  └ ${link.name}` })),
    ...secondaryNavLinks,
  ];
      ...
      <nav className="sticky top-0 z-50 bg-card/95 backdrop-blur-md border-b border-border shadow-soft">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-16 md:h-20 lg:h-24">
            ...
            {/* Desktop Navigation */}
            <div className="hidden lg:flex items-center gap-1 flex-1 justify-center max-w-3xl mx-auto">
              {/* Main Links */}
              ...
              >
                <button
                  className={`relative text-sm xl:text-base transition-all duration-300 font-medium px-3 py-2 rounded-lg whitespace-nowrap flex items-center gap-1 ${
                    isBusinessActive() 
                      ? 'text-primary bg-primary/10 border border-primary/30' 
                      : 'text-muted-foreground hover:text-primary hover:bg-primary/5'
                  }`}
                ...
                {businessDropdownOpen && (
                  <div className="absolute top-full left-0 mt-1 w-56 bg-card/98 backdrop-blur-xl border border-border rounded-xl shadow-lg py-2 z-50">
                    <PrefetchLink
                      to="/business"
                      className="block px-4 py-2.5 text-sm font-medium text-foreground hover:text-primary hover:bg-primary/5 transition-colors"
                    >
          ...
          <div className="lg:hidden fixed top-16 md:top-20 lg:top-24 left-0 right-0 bottom-0 bg-card/98 backdrop-blur-xl border-t border-border shadow-2xl z-50 overflow-y-auto">
            <div className="container mx-auto px-4 py-6 space-y-2">
              {allMobileLinks.map((link) => {
                const isActive = isActiveLink(link.href);
                const isSubLink = link.name.startsWith('  └');
                return (
                  <Link
