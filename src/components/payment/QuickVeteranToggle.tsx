...
export function QuickVeteranToggle({
  isVeteran,
  onToggle,
  onVeteranChange,
  discountPercent = 10,
}: QuickVeteranToggleProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <div
          className={`p-2 rounded-lg transition-colors ${isVeteran ? "bg-primary/10" : "bg-muted"}`}
        >
          <Shield
            className={`h-4 w-4 ${isVeteran ? "text-primary" : "text-muted-foreground"}`}
          />
        </div>
        <div>
          <Label
            htmlFor="veteran-toggle"
