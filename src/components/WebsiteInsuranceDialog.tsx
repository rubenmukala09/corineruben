...
export const WebsiteInsuranceDialog = ({
  ...
  const pricePercentage = useMemo(() => {
    return Math.min(
      ((totalPrice - BASE_PRICE) / (MAX_PRICE - BASE_PRICE)) * 100,
      100,
    );
  }, [totalPrice]);
  ...
  const handleSubscribe = async () => {
    ...
    const finalPriceId =
      matchingPackage?.priceId || "price_1Sjp3WJ8osfwYbX7cbaYjIqU";
  ...
  const groupedFeatures = useMemo(() => {
    return {
      ...
      performance: insuranceFeatures.filter(
                        ...
                        <div className="space-y-1.5">
                          {features.map((feature) => (
                            ...
                            >
                              ...
                              <span
                                className={`text-xs font-medium ${feature.price === 0 ? "text-success" : ""}`}
                              >
                                {feature.price === 0
                                  ? "Included"
                                  : `+$${feature.price}`}
