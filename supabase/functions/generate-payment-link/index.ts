...
serve(async (req) => {
  ...
  try {
    ...
    logStep(`Request ${requestId} - Parsed request`, { 
      ...
      customerEmail: customerEmail ? `${customerEmail.slice(0,3)}***` : 'not provided',
      customerName: customerName || 'not provided',
      items: items.map((i: CartItem) => ({ name: i.name, qty: i.quantity, price: i.price }))
    });
    ...
    const calculatedTotal = items.length > 0 
      ? items.reduce((sum: number, item: CartItem) => sum + (item.price * item.quantity * 100), 0)
      : amount;
    ...
    try {
      ...
      logStep(`Request ${requestId} - Analytics logging failed (non-critical)`, { 
